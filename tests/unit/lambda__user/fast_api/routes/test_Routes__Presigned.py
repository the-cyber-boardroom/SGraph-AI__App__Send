...
class test_Routes__Presigned(TestCase):
    ...
    def test__capabilities(self):
        response = self.client.get('/presigned/capabilities')
    ...
    def test__initiate__memory_mode(self):
        ...
        create_resp = self.client.post('/transfers/create',
                                       json=dict(file_size_bytes=1024))
        tid = create_resp.json()['transfer_id']

        response = self.client.post('/presigned/initiate',
    ...
    def test__complete__memory_mode(self):
        response = self.client.post('/presigned/complete',
    ...
    def test__abort__memory_mode(self):
        response = self.client.post('/presigned/abort/fake123456ab/fake-upload')
        assert response.status_code      == 400
    ...
    def test__upload_url__memory_mode(self):
        create_resp = self.client.post('/transfers/create',
                                       json=dict(file_size_bytes=1024))
        tid = create_resp.json()['transfer_id']
        response = self.client.get(f'/presigned/upload-url/{tid}')
        assert response.status_code      == 400
    ...
    def test__download_url__memory_mode(self):
        ...
        response = self.client.get(f'/presigned/download-url/{tid}')
    ...
    def test__download_url__not_found(self):
        response = self.client.get('/presigned/download-url/nonexistent12')
...
class test_Routes__Presigned__Capabilities_Details(TestCase):
    ...
    def test__capabilities__types(self):
        data = self.client.get('/presigned/capabilities').json()
