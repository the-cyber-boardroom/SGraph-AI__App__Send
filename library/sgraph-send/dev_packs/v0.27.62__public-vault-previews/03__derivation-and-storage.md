...
> **✅ RESOLVED (25 May) — Option A implemented (per-transfer `allow_recreate` flag).** A blocker was found and fixed during implementation: `delete_transfer` used to always leave a `status:'deleted'` metadata tombstone, which blocked recreate at the same id (409). The project lead chose a small, **opt-in** backend change: `create_transfer` now accepts **`allow_recreate`** (`Schema__Transfer.py`, default `False` = unchanged tombstone behavior). When a transfer is created with `allow_recreate:true`, `delete_transfer` **clears the metadata entirely** (returns `recreatable:true`) so the same transfer-id can be recreated/overwritten. `publishPreview` sets `allow_recreate:true`, so **delete-then-recreate works and the share link is unchanged across edits.** Verified: `tests/unit/lambda__user/service/test_Transfer__Service__public_preview.py` (tombstone for default; recreate for opt-in) + 66/66 existing transfer tests still pass.
...
- Composes cleanly with the sub-vaults `.vault/owner/` convention (doc 02 §4.4): `.vault/owner/public-previews/` is a sibling conventional subtree.
