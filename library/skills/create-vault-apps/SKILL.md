...
| **Append-only lanes** — send messages/telemetry to another vault, or receive them from other agents | `sg.append.{configure,write,list,fetch,markProcessed,purge}` (renamed from `sg.inbox.*` 2026-06-15; the write verb is `write`, not `append`) | Requires an `append.*` grant in `app.json` `permissions` — booleans, default-deny, **not** path-scoped. Watch three things. **(1) `fetch` is gated by `append.read`**, not `append.fetch`. **(2) Only `write` crosses vaults** — it takes an explicit `vault_id` and is the account-less delivery path; `list`/`fetch`/`markProcessed`/`purge`/`configure` all act on the *currently open* vault and ignore any vault you name, so **you cannot drain a remote lane from an app** (by design — the `enum_key` is the recipient's). **(3) A read-only session does NOT block `sg.append.write`** — the grant is the only gate, and `sg.app.writable` is irrelevant here; it's the *read* verbs that fail closed without a derivable `enum_key`. `inbox` is the lane id from `list` output — pass it back verbatim, and note it is currently the append token itself. See AUTHORING.md → "Receiving messages". |
| **Calling a third-party API directly** — an external host, not a vault path | Blocked: the frame ships `<meta http-equiv="Content-Security-Policy" content="connect-src blob: data:">` | **No network host is reachable from your frame.** `fetch`/XHR/WebSocket to any `http(s)://` origin fail in ~1–15 ms ("Load failed") and fire `securitypolicyviolation` with `effectiveDirective: "connect-src"` — this is not a 404 and no amount of retrying fixes it. The escape hatch is `"permissions": {"network": true}`, which omits the meta entirely and shows a standing "direct network access" chip in the HUD. **Treat it as a last resort** — it reopens egress to *every* host from a frame holding decrypted vault content. Check first whether an `sg.*` namespace already covers it: `sg.llm.*` for models, `sg.append.write` for delivering into another vault's lane. Those run in the host, keep credentials out of your frame, and need no CSP change. See AUTHORING.md → "Egress". |
...
**And `fetch()` of an *external* host fails for a different reason** — the frame's CSP
(`connect-src blob: data:`) blocks it outright. That one is not a 404: it fails in
milliseconds with "Load failed" and a `securitypolicyviolation` event. Don't debug it as a
routing problem. Either use the `sg.*` namespace that covers what you need (`sg.llm.*`,
`sg.append.write`), or declare `"permissions": {"network": true}` and accept that you have
reopened egress to every host from a frame holding decrypted vault content.
