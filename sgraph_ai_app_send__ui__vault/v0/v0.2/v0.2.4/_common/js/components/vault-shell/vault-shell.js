...
(function() {
    ...
    VaultShell.styles = `
        ...
        /* Phone-first (floor 390px). Only fires in Mobile view; Desktop view lays
           out at 1280px so these never match — the user zooms out instead. */
        @media (max-width: 600px) {
            /* The debug pane is 360px fixed — cap it so it can't exceed the screen. */
            .vs-debug-sidebar { flex-basis: 88vw !important; min-width: 0; max-width: 88vw; }
            /* Settings sub-tabs scroll horizontally rather than overflow the row. */
            .vs-stabs { overflow-x: auto; -webkit-overflow-scrolling: touch; }
            .vs-stab { white-space: nowrap; min-height: var(--touch-target, 44px); }
            .vs-view-sgit { padding: var(--space-2); }
            /* Sync-notice wraps its action button under the text at narrow widths. */
            .vs-sync-notice { flex-wrap: wrap; }
        }
    `;
