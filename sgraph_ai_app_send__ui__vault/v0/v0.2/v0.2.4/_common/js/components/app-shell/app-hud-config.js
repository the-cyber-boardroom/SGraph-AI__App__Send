/* =================================================================================
   ...
   Pure, DOM-free resolver for the `app.json` `hud.*` schema:
       hud.mode  ∈ {'full', 'minimal', 'hidden'}      (default 'full')
...
(function () {
    ...
    var DEFAULTS_MINIMAL = {
        vaultName:  true,  appTitle:  true,  openVault:  false, copyLink: false,
    ...
    var AppHudConfig = {
        ...
        // - 'hidden' mode resolves to mode='hidden' with show defaults from 'full'
        //   so that IF the user override force-shows the HUD, every button is on
        //   by default — minimal-style stripping only applies to 'minimal' mode.
        ...
        resolve: function (input) {
            ...
            var mode = (rawMode === 'hidden' || rawMode === 'minimal' || rawMode === 'full')
                ? rawMode
                : 'full';
