/* =================================================================================
   ...
   v0.2.4 -- brand, vault name, sync status pill, open-app, overflow menu
            + Desktop/Mobile view toggle (overflow menu) for the viewport escape hatch
...
(function() {
    ...
    class VaultHeader extends HTMLElement {
        ...
        connectedCallback() {
                    ...
                    <div class="vh-right">
                        ...
                        <div class="vh-overflow">
                            ...
                            <div class="vh-menu vh-overflow-menu" style="display:none">
                                <button class="vh-mi vh-mi--viewmode" type="button"></button>
                                ...
                                <div class="vh-version">v0.2.4</div>
                            </div>
                        </div>
                    </div>
            ...
            this.shadowRoot.addEventListener('click', (e) => {
                ...
                if (e.target.closest('.vh-mi--viewmode')) { this._closeMenus(); this._toggleViewMode(); }
            ...
            // Keep the view-mode menu label in sync if the mode is changed elsewhere.
            this._onViewModeChanged = () => this._renderViewModeItem();
            document.addEventListener('sg-vault-view-mode-changed', this._onViewModeChanged);

            this._renderPill();
            this._renderViewModeItem();
            this._fetchAppVersion();
        }

        disconnectedCallback() {
            document.removeEventListener('click', this._onDocClick, true);
            if (this._onViewModeChanged) document.removeEventListener('sg-vault-view-mode-changed', this._onViewModeChanged);
        }
        ...
        // --- Desktop/Mobile view toggle (viewport escape hatch) ---

        _renderViewModeItem() {
            const item = this.shadowRoot.querySelector('.vh-mi--viewmode');
            if (!item) return;
            const vm = window.SGVaultViewMode;
            if (!vm) { item.style.display = 'none'; return; }
            item.style.display = '';
            // Label shows the action (what you'll switch TO), not the current state.
            item.innerHTML = vm.isDesktop()
                ? '📱 Mobile view'      // 📱 — currently desktop, offer mobile
                : '💻 Desktop view';    // 💻 — currently mobile, offer desktop
        }
    ...
    VaultHeader.styles = `
        ...
        .vh-menu {
            ...
            min-width: 230px; max-width: calc(100vw - 24px); padding: var(--space-2);
        ...
        /* --- Phone-first responsive (floor 390px). Only fires in Mobile view; in
               Desktop view the layout viewport is 1280px so these never match. --- */
        @media (max-width: 600px) {
            .vh-header { padding: 0 var(--space-2); gap: var(--space-1); }
            .vh-right  { gap: var(--space-1); }
            /* Reclaim horizontal room: brand + vault name shrink to body size. */
            .vh-brand  { font-size: var(--text-body); }
            .vh-vault-name { max-width: 32vw; font-size: var(--text-body); }
            /* Tap targets — 44px min on the interactive chrome. */
            .vh-pill, .vh-lock-btn, .vh-open-app-btn, .vh-overflow-btn {
                min-height: var(--touch-target, 44px);
            }
            .vh-overflow-btn { min-width: var(--touch-target, 44px); }
            /* "Return to Vaults" label is wide; keep the glyph, drop the word. */
            .vh-lock-btn { font-size: 0; padding: 0 0.6rem; }
            .vh-lock-btn::before { content: '\\21C6'; font-size: var(--text-body); }
            /* Menu rows get comfortable touch height. */
            .vh-act, .vh-mi { min-height: var(--touch-target, 44px); }
            .vh-unlock-input, .vh-unlock-apply { min-height: var(--touch-target, 44px); }
        }
    `;
