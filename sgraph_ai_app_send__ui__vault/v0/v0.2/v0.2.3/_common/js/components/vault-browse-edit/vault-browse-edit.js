...
(function() {
    ...
    SendBrowse.prototype._build = function() {
        ...
        // Add link button (sub-vaults + external resources) — only when the convention
        // reader is loaded (VaultLinks). Creates a *.link.json the composite renders inline.
        if (typeof VaultLinks !== 'undefined') {
            var addLinkBtn = document.createElement('button');
            addLinkBtn.className = 'sb-action-btn';
            addLinkBtn.innerHTML = '&#128279; Add link';   // 🔗
            addLinkBtn.title = 'Link another vault, or embed an external resource';
            addLinkBtn.addEventListener('click', function() { _showAddLink(self); });
            headerRight.prepend(addLinkBtn);
        }
    };
