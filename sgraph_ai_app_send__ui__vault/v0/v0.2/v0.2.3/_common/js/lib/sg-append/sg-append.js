/* =================================================================================
   ...
   NOTE (v0.32.7 rename): inbox → append, end to end. URL paths, the outbound
   verb (now `write`, matching POST /write/{vault_id}), the class/global name
   (SGAppend), and the file/dir name are all updated. Two things are deliberately
   PRESERVED because they are wire/crypto contracts the server depends on:
     • the enum_key derivation prefix "sg-inbox-enum:" (changing it breaks the gate)
     • the `inbox` body field in list/fetch/mark/purge requests (the server retains
       this field name — it identifies the token folder; see migration guide §3).
   ...
   Design notes:
     ...
     - Errors carry a `.code` (EINVAL / EPERM / E2BIG / ENOSPC / EUNREACH / EHTTP) and,
       for HTTP failures, an `.http` status — the checker maps these onto append.error.
...
class SGAppend {
    ...
    static async deriveEnumKey(readKeyBytes) {                                   // readKeyBytes: Uint8Array (raw read_key)
        if (!readKeyBytes || !readKeyBytes.length) {
            throw SGAppend._err('EINVAL', 'read_key bytes required for enum_key derivation')
        }
        ...
        return SGAppend._hex(new Uint8Array(buf))
    }
    ...
    async purge({ folder = 'processed', inbox, file_ids } = {}) {              // owner: delete; empty file_ids + processed = bulk purge
        ...
        if (folder !== 'pending' && folder !== 'processed') {                  // v0.32.7: 'inbox' folder value → 'pending'
            throw SGAppend._err('EINVAL', "folder must be 'pending' or 'processed'")
        }
    ...
    _ownerHeaders() {
        if (!this.writeKeyHex) throw SGAppend._err('ENOAUTH', 'write_key required for this verb (owner session)')
    ...
    static _assertBatch(ids) {
        if (!Array.isArray(ids) || ids.length === 0) {
            throw SGAppend._err('EINVAL', 'file_ids must be a non-empty array')
        }
        if (ids.length > 100) {                                                 // APPEND_BATCH_MAX_FILE_IDS — server rejects; fail fast client-side
            throw SGAppend._err('E2BIG', `file_ids exceeds batch cap of 100 (got ${ids.length})`)
        }
    }
...
if (typeof globalThis !== 'undefined') globalThis.SGAppend = SGAppend
