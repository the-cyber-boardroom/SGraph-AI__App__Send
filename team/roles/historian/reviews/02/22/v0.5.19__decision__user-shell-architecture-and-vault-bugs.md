...
- **Bug #6** — "Generate Key Pair" appears unresponsive (**CORRECTED**: original diagnosis that `extractable: false` caused public key export failure was wrong — public keys are always exportable per W3C spec. The `extractable: true` "fix" was a security regression that was reverted. Real cause: RSA-4096 generation takes several seconds + no error handling in `_generateKeys()`. Try-catch added.)
...
| 3 | **CORRECTED**: Web Crypto `extractable` flag only controls PRIVATE key export — public keys are always exportable per W3C spec. Original Bug #6 misdiagnosis led to changing `extractable: false` to `true`, which was a security regression (reverted). See AppSec review. | Require AppSec review for any change to crypto parameters |
...
- Dev review: `team/roles/dev/reviews/02/22/v0.5.19__review__vault-ui-bugs-and-shell-architecture.md`
- Previous vault architecture: `team/roles/dev/reviews/02/22/v0.5.19__review__pki-vault-revised-cache-service-architecture.md`
