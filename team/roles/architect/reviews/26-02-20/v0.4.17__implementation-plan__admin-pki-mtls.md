...
The `sigsend` CLI tool (from the LLM integration plan) can also handle admin operations:

```bash
# Generate admin key pair
sigsend admin keygen --output admin-key.pem

# Register public key with admin service
sigsend admin register --key admin-key.pem --name "CI Pipeline"

# Make signed admin request
sigsend admin stats --key admin-key.pem
```
