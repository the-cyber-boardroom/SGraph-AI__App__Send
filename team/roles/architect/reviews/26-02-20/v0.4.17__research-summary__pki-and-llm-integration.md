...
**The CLI tool (`sigsend`) should be built first.** It works with every LLM that has shell access, uses the same `cryptography` library for format-compatible AES-256-GCM, and the same code powers the MCP server.
...
|----------|-----------|---------------|
| 1 | CLI tool (`sigsend`) — crypto module, upload/download, PyPI | [LLM Integration Plan](v0.4.17__implementation-plan__llm-integration-cli-mcp.md) |
...
1. **CLI package name:** `sigsend` or `sg-send`?
