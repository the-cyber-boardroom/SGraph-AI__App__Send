...
This plan covers building a CLI tool (`sigsend`) and an MCP server for SG/Send, enabling any LLM with shell or MCP access to encrypt and upload files while preserving zero-knowledge encryption.
...
sigsend/
  __init__.py
  __main__.py          # Entry point: python -m sigsend
...
sigsend upload --file report.pdf
# Output: JSON with download_url, transfer_id, key

# Download and decrypt a file
sigsend download --url "https://send.sgraph.ai/send/...#transfer_id/key" --output report.pdf
# Or: sigsend download --transfer-id abc123 --key <base64url-key> --output report.pdf

# Get transfer info
sigsend info --transfer-id abc123

# Health check
sigsend health --server https://send.sgraph.ai
```
...
- **PyPI:** `pip install sigsend`
- **uvx:** `uvx sigsend upload --file report.pdf` (no install needed)
...
from sigsend.crypto import encrypt_file, decrypt_file
from sigsend.api_client import upload_encrypted, download_encrypted
...
|------|-------------|--------|--------------|
| 1 | `sigsend` crypto module + tests | 1 day | None |
| 2 | `sigsend` API client + upload command | 1 day | Step 1 |
| 3 | `sigsend` download command | 0.5 day | Step 1 |
...
1. **Package name:** `sigsend` or `sg-send` on PyPI?
2. **MCP bundling:** Separate `sigsend-mcp` package or `sigsend mcp` subcommand?
