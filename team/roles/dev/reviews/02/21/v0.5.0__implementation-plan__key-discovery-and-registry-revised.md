# v0.5.0 — Implementation Plan: Key Discovery & Registry (REVISED)

**Version:** v0.5.0
**Date:** 2026-02-21
**Role:** Developer
**Team:** Explorer
**Status:** Revised based on human decisions — ready for implementation
...
1. **All components loaded inside the admin shell** — no standalone pages
2. **IFD architecture** — EventBus, Shell+Router, Web Components (light DOM), Services
3. **3 debug side panels** — API Invocations, UI Events, UI Messages (from Issues-FS)
4. **Refactor pki.html** — break into separate components, each doing one thing, communicating via events
5. **New version v0.1.3** — per IFD methodology, new features = new version
6. **Case-insensitive lookup codes** — normalise to lowercase at API boundary
7. **Backend data browser** — expose Memory-FS content for visual debugging
8. **API-first** — everything exposed via REST API (admin-only), which powers the UI
9. **All endpoints on Admin Lambda** — already protected by Fast_API cookie auth

---

## 1. Non-Negotiable Constraints

1. **IFD architecture** — EventBus for all component communication, Shell+Router for navigation, light DOM (no Shadow DOM), self-contained components
...
4. **All tests use real implementations** — no mocks, no patches, in-memory Memory-FS
5. **No plaintext names on server** — only Obj_Ids, PEM keys, timestamps, codes
6. **Case-insensitive lookup codes** — normalise to lowercase at API boundary, display uppercase
7. **New version v0.1.3** — previous versions (v0.1.0, v0.1.1, v0.1.2) are frozen
8. **3 debug panels required** — API Invocations, UI Events, UI Messages
...
**Namespace:** `window.sgraphAdmin` (not `window.issuesApp` — this is the admin console).
...
│ ├ Contacts│  <pki-keys>     (key mgmt)    │  <events-viewer>            │
│          │  <pki-encrypt>   (encrypt/dec)  │  <api-logger>              │
│ Registry │  <pki-contacts>  (contacts)     │                             │
│ ├ Publish│  <key-publish>   (publish flow) │                             │
│ ├ Lookup │  <key-lookup>    (lookup by code│                             │
│ ├ Browse │  <key-registry>  (all keys)     │                             │
│          │  <key-log>       (transparency) │                             │
│ Tokens   │                                 │                             │
│ ├ Manage │  <token-manager> (existing)     │                             │
│          │                                 │                             │
│ Storage  │  <storage-browser> (Memory-FS)  │                             │
│ ├ Browse │                                 │                             │
│          │                                 │                             │
│ System   │  <system-info>   (existing)     │                             │
│ ├ Info   │  <cache-browser> (existing)     │                             │
│ ├ Cache  │                                 │                             │
│          │  <analytics-dashboard> (exist.) │                             │
│ Monitor  │  <metrics-dashboard>  (exist.)  │                             │
│ ├ Metrics│                                 │                             │
├──────────┴─────────────────────────────────┴─────────────────────────────┤
│ Ready │ Active: My Keys │ Components: 12 │ [Debug Panel 🔴2]           │
└──────────────────────────────────────────────────────────────────────────┘
```
...
| `key-lookup` | `<key-lookup>` | Look up key by code | — | `key-imported` |
| `key-registry` | `<key-registry>` | Browse all published keys | `key-published`, `refresh` | `key-imported` |
| `key-log` | `<key-log>` | Transparency log viewer | `key-published`, `refresh` | — |
...
### 3.2 Storage Browser Endpoints (NEW — admin-only)
...
These endpoints expose the Memory-FS backend for admin debugging. They show exactly what's stored on the server — encrypted blobs, tokens, key entries, log entries. This is critical for development visibility.
...
**Goal:** Create the v0.1.3 admin UI with IFD architecture. Migrate existing components. Add the 3 debug panels. No new features yet — just the architecture.
...
| `v0.1.3/js/keys-service.js` | KeysService — API client for key CRUD, wraps IndexedDB + REST |
...
User generates a key in <pki-keys>
  → pki-keys emits 'key-generated' { keyId, fingerprint }
    → pki-encrypt listens, updates "My Keys" dropdown
    → events-viewer shows event
    → messages-service shows "Key generated" success

User clicks "Publish" in <pki-keys>
  → pki-keys emits 'key-selected' { record }
    → key-publish listens, shows publish form
    → keys-service calls POST /keys/publish
      → api-logger records the call
      → key-publish emits 'key-published' { code, fingerprint }
        → pki-keys updates key card with "Published as DC-7X4F"
        → key-registry refreshes
        → key-log refreshes
```

**Verification:** All existing PKI functionality works — generate key, export, import contact, encrypt, decrypt, sign, verify. Events flow correctly through EventBus.
...
| File | Purpose |
|------|---------|
| `v0.1.3/components/key-publish/key-publish.js` | Publish flow (select key → confirm → get code) |
| `v0.1.3/components/key-lookup/key-lookup.js` | Lookup by code (enter code → view key → import) |
...
test__lookup_key                    — GET by code → full entry
test__lookup_key__case_insensitive  — DC-7X4F = dc-7x4f = Dc-7x4F
test__lookup_key__not_found         — unknown code → 404
test__unpublish_key                 — DELETE → status unpublished
test__unpublish_key__already_gone   — unpublish twice → 404
...
test__log_integrity                 — each entry's previous_hash matches prior
...
- Shows tokens, keys, transfers, log entries — everything stored on the server
...
**Key rules:**
- **Light DOM** (no Shadow DOM) — enables surgical overrides and style cascading
- **IIFE pattern** — keeps internal state private
- **Bound handlers** — store in `_boundHandlers` so you can `off()` them
- **Always cleanup** — every `on()` must have a matching `off()` in `disconnectedCallback()`
- **CSS prefix** — use short prefix per component (e.g., `pk-` for pki-keys, `kl-` for key-lookup)
...
| AppSec: read now | `team/roles/appsec/reviews/02/21/v0.5.0__appsec__read-now.md` |
| AppSec: future findings | `team/roles/appsec/reviews/02/21/v0.5.0__appsec__future-findings-catalogue.md` |
| GRC risk: auth transition | `team/roles/appsec/reviews/02/21/v0.5.0__grc-risk__auth-model-transition.md` |
| Original impl plan | `team/roles/dev/reviews/02/21/v0.5.0__implementation-plan__key-discovery-and-registry.md` |
