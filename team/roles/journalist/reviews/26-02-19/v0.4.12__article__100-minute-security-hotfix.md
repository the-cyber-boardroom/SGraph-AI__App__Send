...
```
https://send.sgraph.ai/.../download.html?token=linkedin-user#16bff884b7da/dptbfg...
```

That `?token=linkedin-user` is an access token. The sender's access token. Sitting in the query string of every shareable link the platform had generated since version 0.1.3.
...
```
https://send.sgraph.ai/.../download.html#f00e921c2bf9/3BuFuDdzMscdz2NmOrilWOIyMtq09WulkzzfpGPVLZU
```
