...
| Reuses SG/Tools `core/sg-audio` (`getBestMimeType`) + `core/sg-audio-decode` (`blobToWav`) by lazy `import()` — **we do not implement recording or audio decoding**. iPad records `m4a` → sent as-is; Chrome records `webm` (not accepted by OpenRouter) → transcoded to WAV | **EXISTS** | `sg-voice.js`; `test__sg_voice.js` (67 assertions) |
