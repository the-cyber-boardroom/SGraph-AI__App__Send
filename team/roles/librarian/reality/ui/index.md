...
**Domain:** `ui/` | **Last updated:** 2026-08-04 | **Maintained by:** Librarian (daily run)
...
#### Vault tools in the chat panel — B1 (2026-08-06)

Plan: `team/roles/architect/reviews/08/06/v0.33.56__architect-review__vault-chat-tools-convergence.md`.
Changelog: `team/comms/changelog/08/06/v0.33.56__changelog__vault-chat-tools-b1.md`.

| Capability | Status | Evidence |
|------------|--------|---------|
| **`tool_calls` survive the `SGLlm` transport** — streamed fragments reassembled by index (split names/arguments concatenate), JSON path read, `finish: 'tool_calls'`, malformed arguments → `args: null` + `argsRaw` kept, ids synthesised when a route omits them. `toolCalls === null` on plain replies | **EXISTS** | `sg-llm.js`; `test__sg_llm.js` (69 assertions) |
| **`SGLlmTools`** — opt-in tool groups for the panel. **Default is NOTHING**: no `tools.json` → every group off → `compileTools() === []` → zero tokens added. B1 groups: `session` (get_costs, get_exchanges from `VaultLlmLog`) and `files.read` (list_folder, read_file, stat, exists) | **EXISTS** | `_common/js/lib/sg-llm/sg-llm-tools.js`; `test__sg_llm_tools.js` (51 assertions) |
| **Grants persist in `/.vault/llm/tools.json`** — separate from `config.json` (which carries the sealed key); every toggle is a commit, so `sgit log` of that one path is the privilege audit log. Read-only vault → `EREADONLY` on save; corrupt file fails CLOSED | **EXISTS** | `SGLlmTools.loadGrants/saveGrants`; panel `_setGroupEnabled` reverts on failed save |
| **The floor beats every grant** — `.vault/**` refused `EPROTECTED` at dispatch, including traversal/backslash spellings and **the grants file itself** (no self-inspection, no self-escalation). `.vault` never appears in `list_folder` results | **EXISTS** | `SGLlmTools.pathAllowed/isFloor/dispatch`; tests |
| **Per-group path scope** — allow/deny globs (`docs/**`, `*.md`; `*` does not span segments, deny wins, empty allow = everything minus floor), enforced at dispatch AND stated in the tool description the model receives. Root stays listable under an allow-scope so navigation works | **EXISTS** | `globToRegExp`/`pathAllowed`/`_scopeNote`; tests |
| **`read_file` returns FENCED content** (`╔═ BEGIN UNTRUSTED DATA ═╗ … ╚═ END ═╝`, same format as the vault-chat pack) with truncation stated inside the fence; the system prompt gains the fence rule only when tools are enabled | **EXISTS** | `SGLlmTools.fence`/`SYSTEM_TOOLS_NOTE`; `test__vault_llm_chat.js` |
| **The tool loop in the panel** — send → dispatch → resend until prose; every round is its **own `VaultLlmLog` entry**; tool calls render as transcript chips while they run (`🔧 read_file docs/plan.md ✓`); only final prose joins the conversation history (fenced file bodies are per-turn plumbing, not resent context) | **EXISTS** | `vault-llm-chat.js` `_chatOnce`/`_runTools`; `test__vault_llm_chat.js` (149 assertions) |
| **Loop termination** — `MAX_ITERATIONS` (8) rounds, then one final **tool-less** round so an answer still lands, with the cap stated to the user | **EXISTS** | `_send` loop; termination suite |
| **The `🔧` bar** — groups off by default, tier + measured token weight per group (`~N tok/req`, computed from the JSON actually sent), commit-as-audit-log stated; a failed save **reverts the toggle** (the UI never claims authority the vault refused); read-only vault shows the granted state locked with the reason | **EXISTS** | `toggleTools`/`_renderTools`/`_setGroupEnabled` |
| Not built (B2/B3): `ui` + `history` groups, Settings → AI Tools page (scope editor with live match counts, audit view), `files.write` via working set + one-commit flush | **PROPOSED** | the architect review §6 |
