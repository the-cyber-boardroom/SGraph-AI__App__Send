...
- **In-vault nav row V1 (SHIPPED 30 May 2026, `claude/wizardly-bell-wcBUT`).** Browser-style toolbar inside the HUD: back / forward / refresh arrows + path display (with `#fragment` highlighted in teal) + copy-path button + ⋯ recent-pages menu (last 12, move-to-front). The HUD never owns history; it dispatches `app-hud:nav` `{action: back|forward|reload|jump|exit, path?}` and renders state pushed back via `app-nav:change` from `<app-shell>`. Type-to-jump address-bar autocomplete is **NOT** in V1 — deferred to a follow-up. **Companion bug-fix:** the iframe click interceptor in `app-shell._buildVfsBridgeScript` now strips `?`/`#` before its `.html`/`.htm` extension check, so `pages/x.html#section` links are intercepted instead of falling through to a real GET and 403-ing on the static host (the visible "AccessDenied" dead-end the user reported on `dev.vault.sgraph.ai`). The fragment is forwarded to the parent, which posts `{__sgVfsScrollToHash: "section"}` to the new srcdoc after the swap; a bridge listener applies it on `DOMContentLoaded` (null-origin safe). Friendly broken-link / access-denied overlay replaces the prior `console.warn`-only dead-end (`_renderBrokenLinkOverlay`, rendered as a srcdoc inside the same iframe so the HUD's ‹ back arrow can escape).
- **`app.json` `hud.*` config schema (SHIPPED 30 May 2026).** `hud.mode ∈ {full, minimal, hidden}` (default `full`) + `hud.show.{vaultName,appTitle,openVault,copyLink,print,debug,navBar,navArrows,navPath,navRefresh}` granular flags resolved by `AppHud._resolveHudCfg` (pure static; per-mode defaults then explicit overrides). `mode:"hidden"` collapses the entire HUD; the iframe takes 100% of the viewport. **Sovereignty rail (apps cannot suppress):** (1) active `requestConsent` overlays always render; (2) a corner `× Exit app` pill (top-right, `position:fixed; z-index:9999`) persists in `hidden` mode and routes to `/en-gb/vault/`; (3) user-side override `localStorage['sg-app-force-show-hud']='1'` forces `mode:"full"` regardless of `app.json` (read by the page script in `en-gb/app/index.html`, not bypassable by apps). Applied via `hud.applyHudConfig(cfg)` called by the page script on `app-shell:ready` (cfg is in the event detail).
- **New `<app-shell>` nav surface** to support the HUD: `_navigateToPath(href, {pushHistory})`, `_pushNavHistory`, `_navBack`, `_navForward`, `_navReload`, `_canNavBack`, `_canNavForward`, `_currentNavPath`, `_renderBrokenLinkOverlay(path, reason∈{missing,blocked})`, `_exitApp`, `_emitNavChange`. The legacy `__sgVfsNavReq` postMessage handler now delegates to `_navigateToPath`. Nav history is seeded with the entry path on `_mountApp` (line 1190-ish), then pushed onto by every successful in-vault link click. Browser semantics: forward stack is truncated when a new nav arrives.
- **Print-button regression (KNOWN, not fixed in V1).** The Print button added on this branch (`57de4e67`) reads `iframe.contentDocument`, which throws `SecurityError` under ViV Phase 3 null-origin srcdoc frames. The HUD config default now hides it (`show.print: false`); a follow-up Commit B will add a bridge-RPC print path (app posts DOM snapshot via SecureChannel; parent reconstructs and prints) and flip the default back to `true`.
