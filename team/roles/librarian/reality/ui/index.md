...
Collapsible right-edge debug panel with 6 tabs: Vault Trace, Bridge Log, Mounts (single-kernel, B4), **Audit** (cross-kernel, Phase 5.1), App State, Network. Collapses to a narrow edge strip when host width < 40 px (ResizeObserver). Tabs are lazy-loaded sub-components.
