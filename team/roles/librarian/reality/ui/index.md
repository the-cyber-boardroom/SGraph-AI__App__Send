...
- ~~**Open item — send-browse v0.3.3 `_write` encoder chunk bug**~~ — **FIXED 2026-08-03.** `send-browse--v0.3.2.js` (user v0.3.3) and `send-browse--v0.3.3.js` (open v0.4.0) both chunked base64 at 8192 in the injected `sg.vfs.write` bridge. `8192 % 3 === 2`, so each separately-encoded slice emitted `=` padding **mid-string** and `atob()` — which accepts `=` only at the end — failed or truncated. **Every write over 8 KB from the /vault preview pane was silently corrupted, and the corruption was persisted into the vault**, so an affected JS file never parses again ("missing ) after argument list"). Now 8190 (= 3×2730). Guarded repo-wide by `tests/unit/vault_ui/loader/test__base64_chunk_guard.js`, which fails with file:line if any chunked base64 encoder uses a size not divisible by 3 — this bug had shipped three times in three files. **Deploy note:** these files are served from `dev.send.sgraph.ai`, so the fix ships via the **user/open UI pipelines**, not the vault one.
