...
| Open PROPOSED items (across all domains) | ~1112+ (+26 from 09/23 session: P-CEDAR-001/002, P-INS-001/002, P-GAME-003–008, P-LTO-001/002, P-SB-001, P-PM-002, P-AIUC-001/002, P-GM-REPO-001, P-ASSESS-001, P-CSYNC-001, P-VTV-001/002/003, P-SITES-002, P-PROV-001, P-PG-001, P-VID-001; prev: ~1086+) |
| Total documents catalogued | 1023 (+43 from 26 Aug–7 Sep 2026: 38 briefs + 5 day indexes; processed 09/23; prev: 980) |
...
| **Key decision** | **Adopt Cedar as mandate policy language** — Cedar (AWS) evaluates outside the agent's reasoning loop; default-deny; explicitly does not establish identity (composes with registry rather than competing); grant is discovered by measurement not authored; only metered primitives may appear in a policy (26 Aug 2026) |
| **Key decision** | **Premium paid in pool allocation, not money** — agent insurance: premium = allocation from finite pool; scarcity is the feature; correlation breaks pooling (shared model/prompt/tooling/deps means draws are not independent); experience rating on team, not on agent that is indifferent to loss; recoverability decides insurability — irreversible stock has hard boundary not high buffer (insurance architecture, 2026-08-26) |
...
| **Key decision** | **Licence derived, never stored** — licence-to-operate computed on every render from three inputs (insurance owned + mandate in force + zone not outside); the most important word on screen is in no file; three revocation paths (amount, scope, lapse) must look different on screen (licence-to-operate vault, 2026-09-03) |
| **Key decision** | **Status block cannot be a meter** — a self-report of session consumption is consistently low by ~4× in one direction and cannot fix its own baseline; the calibration instrument's product is the gap per model between two independent producers of the same object (observation design, 2026-09-03) |
| **Key decision** | **Scope check before amount check** — an action outside the mandate is refused on capability, meter never read; this produces an audit log entry of the type "capability denied", not "metering event", which reveals unauthorised actions rather than disguising them as expensive ones (licence-to-operate vault, 2026-09-03) |
| **Key decision** | **A page is not a vault** — stale-clone rule: clone the thing and check its version log rather than its landing page; reading an index once is not tracking an estate; four documents were written against a fork that had moved (methodology correction, 2026-09-04) |
| **Key decision** | **Two-edge rule is a test, not a preference** — the AIUC-1 conformance layer holds the rule (layer edge must never reach source observation) as a failing build test; a rule held only in documentation is an editorial preference (conformance layer, 2026-09-04) |
...
| **Key decision** | **Probe not row as contribution unit** — the grant-mandate public repository accepts probes (a claim that travels with the command that establishes it) rather than rows; a challenge is a rerun of the probe, not an argument about the row; mandates are elicited locally and never leave the clone (public repo architecture, 2026-09-04) |
| **Key decision** | **Adopt OSCAL and SARIF as interchange** — two published standards already exist at both ends of the assessment chain; one internal shape emits both rather than minting a third schema; import (OSCAL in) prioritised over export (SARIF out) (assessment product, 2026-09-05) |
| **Key decision** | **Reach is a node, not a rung** — a capability's reach is a node in a mesh rather than a position on a ladder; reading a file system is six different exposures wearing one verb; every answer is already a prediction; proper scoring rule replaces correctness because real grants are consistently wider than people expect (game series, 2026-09-05) |
| **Key decision** | **Append lane sits outside the commit DAG** — this single fact settles the logging architecture; a committed write to a folder of N entries rewrites O(log N) ancestors in the Merkle tree; a flat put to an append lane is O(1); difference is not a constant factor but structural (vault logging architecture, 2026-09-07) |
| **Key decision** | **Reply address travels inside the first message** — no directory; the sender carries its own reply address; broadcast is the private option (addressing somebody means learning who they are); a path with one writer and defined readers is an interface (vault-to-vault communication, 2026-09-07) |
| **Key decision** | **These sites are narrower than vendor consoles, not better** — narrowness is the advantage; the differentiator is scope, not feature completeness; a missing feature in the narrower scope is not a gap, it is a boundary (provider sites, 2026-09-07) |
| **Key decision** | **A grant is a research subsidy, not inventory** — two providers now restrict resale; a commercialisation model built on margin over a provisioned key is broken; the funded-key mechanism (invalidated 2026-08-06 for OpenRouter) is now generalised to other providers (provider sites, 2026-09-07) |
| **Key decision** | **Read documentation for the product, not the vendor** — 5 Sep brief quoted signed-URL rules from an agent endpoint; 7 Sep found speech endpoint has no signed URL support; would have produced a broken integration; the 4 Sep stale-clone rule in a new form (methodology correction, 2026-09-07) |
