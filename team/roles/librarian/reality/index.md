...
| Open PROPOSED items (across all domains) | ~1085+ (+12 net-new from 09/08 session: P-GMT-001/002, P-FIX-001/002, P-RUI-001, P-OBS-001, P-SYN-001, P-SHD-001, P-IGN-001, P-CSR-001/002, P-CSL-001; +6 from 08/25 session: P-NHI-001–006; +16 from 08/23 session: P-ENR-001–009, P-BRK-001–007; previously ~1051+) |
| Total documents catalogued | 980 (+18 from 20 Aug: 17 briefs + 1 day index, v0.33.61; processed 09/08; prev: 962) |
...
| **Key decision** | ~~**The grant is the mandate**~~ **CORRECTED 2026-08-20** — the accounting identity stands (you carry the exposure of what you hold) but the inference that they are one object does not. Grant = union of capabilities conferred at assignment (fact). Mandate = what the party is authorised and expected to do (document, 5 fields). Excess authority = grant minus mandate = blast radius from the other end; unaccepted by construction; escalates unaided. Original: "never-in-line" (2026-07-23); Correction: doc 964, v0.33.61, 2026-08-20. |
...
| **Key decision** | **Grant/mandate split** — a grant is the union of capabilities conferred at assignment (fact about the credential); a mandate is what the party is authorised and expected to do (document with 5 required fields: issuer, subject, scope, interval, revocation path); excess authority = grant minus mandate; unaccepted by construction, defaults to critical, escalates without escalating; a declared mandate is instrumentation, not enforcement; an execution broker makes grant = mandate by construction (2026-08-20) |
| **Key decision** | **Fixture class for published keypairs** — a keypair with a published private half provides no authentication, permanently; these objects are fixtures (a class, not a weaker identity); fixtures cannot be promoted, cannot be revoked via the register's signed-append mechanism, must never be reachable from the real trust graph; `private_key_published` is a required first-class field in the register entry schema, no default (2026-08-20) |
| **Key decision** | **Evidence is appended by the asserter to its own record, never the subject's** — derived from 2019 keyserver failure; prevents the flooding and attachment problem; A vouches for B by writing in A's own record; only the owner writes to their own record (pki.sgit.ai four rules, confirmed 2026-08-20) |
| **Key decision** | **Publish the record, never the verdict** — for the consumer transparency register (and by extension for all publishable registers): a dated, verbatim record of what was asked and what was answered is protected by substantial truth; a characterisation requires honest-opinion defence; the defence is cheapest by design, not by litigation; no adjectives, no scores, no labels (2026-08-20) |
| **Key decision** | **Subscription is a discount, not rent** — a subscription is a discount for committing to regular use; not-using is not a refund right; the consumer question is "do you hold a record of my usage?" (answerable by every company, comparable, publishable on day one) not "how much did I use?" (usually unprovable from consumer side); exit-path measurement requires no company cooperation and can ship first (2026-08-20) |
| **Key decision** | **Synthetic readers find defects, not preferences** — the screenshot-only boundary is the validity condition for persona simulation (pixels only; no page text, structure, or source); patience budget must be exogenous (set before the run, spent by mechanical rule); the page under test must be a fixed artefact authored before the run; the same model must not generate the page and react to it (2026-08-20) |
