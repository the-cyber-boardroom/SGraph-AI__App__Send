...
| # | Feature | One-Line Description | Source |
|---|---------|---------------------|--------|
| P-404 | Agent mandate Wardley map series | Eight maps, each anchored at a user need and drawn twice (reality today vs. with our service); user needs: exec (use AI safely), understand/propagate risk, delegate mandates without liability, agent-as-user (ask/receive/check permissions, decide, execute), security team (reduce risk, integrate tools), financial team (control agentic cost), vendor (present credible risk to customers), competitive landscape; recurring insight: components that feel like products are genesis or custom-built (hope-driven authorization is custom, cost control is genesis, the safe uninjected decision is genesis and stays genesis); agent-as-user map is sharpest — execution is commodity, safe decision is genesis; stored in the agent mandate vault graph (P-403) as nodes with `evolution_position` and `visibility_to_user` attributes so map queries are graph queries; first-pass tables authored, to be challenged and visualised as paired before/after graph artifacts; flagship visualisations: exec and agent-as-user; effort: low to moderate | 06/23 wardley-maps/ |
| P-405 | Risk acceptance service MVP | Scenario-picker flow presenting 2–3 hairy, evidence-backed risks with no deny button; five time-boxed intervals (1 hour = need more data / 4 hours = P1 / 2 days = smaller incident / 2 weeks = funded project / 6 months = do nothing, costs zero) each encoding a distinct meaning and cost-of-action; risks can compound and roll up to a top risk, with a stated control adjusting the level; each acceptance expires at its interval and must be renewed; demo rests on the multi-persona agent mandate graph (P-403) and an evidence structure; per-persona customised views backed by real incidents; includes the risk that not all required stakeholders have approved (a mandate-completeness risk); optional Gen AI layer metered via OpenRouter key; vault connection optional to start; first product delivery of Risk Mandate.ai; effort: moderate | 06/23 risk-mandate-product-and-workflow/risk-acceptance-service-brief |
| P-406 | Multi-stakeholder risk acceptance workflow | Two-dimensional acceptance model (direction: get-more-data/reduce/increase/hold; revisit interval: when acceptance expires and must be renewed, independently); exec decisions require underwriting — recorded acceptance or refusal — from the relevant direct-line owner (CIO/CTO/CFO by dimension), the CSO, and GRC before the exec may act; accepted risks propagate upward to the exec's manager, their manager, the CEO (acting for the board), and sometimes the board itself; evidence can strike intervals off as physically impossible — those options disappear from the set for everyone; a superior can override in either direction (tighter or looser) with the subordinate's original acceptance preserved and the override attributed; no override can select a struck-off interval; the only escape from a physically impossible constraint is ceasing the activity; approval attaches to a risk profile (not each instance) — further instances are pre-approved as FYI only; re-approval required when the risk profile changes; the whole workflow is a graph (nodes: risks, stakeholders, acceptances; edges: underwriting, propagation, override, pre-approval); requires ontology extension: `RiskProfile`, `UnderwroteBy`, `PropagatedTo`, `OverriddenBy`, `EvidenceConstraint` entities; effort: significant | 06/23 risk-mandate-product-and-workflow/risk-acceptance-workflow-brief |
| P-408 | Agentic freelancing workflow | Handover agent on the client side collects and packages briefing materials guided by the freelancer's published skills; skills encode how the freelancer works, what they need, and the questions they ask (SKILL.md variant); client provides a brief shaped by those skills; micro-engagement (2-hour) is viable because onboarding cost is near-zero with agents on both sides; mutual benefit: client pays only for the moment of expertise; provider sells only productive time; vault-based handover scopes and protects materials per engagement; goal is to pilot and then guide customers into the workflow so it scales; effort: low to moderate | 06/23 agentic-freelancing/ |
| P-409 | Personal risk acceptance scenario | Curated question graph walking a personal user through the local-LLM scenario; six questions (using Claude; YOLO vs. accept-each; account; on-disk credentials; logged-in accounts; data-on-machine-not-yours) typed as fact/opinion/hypothesis/evidence; first-pass evidence stored in browser local storage (not vault), with the user told it is local; risk graph generated from answers (not generic risks); two concrete chains: (1) lethal trifecta — if reads untrusted content AND internet-connected AND can act autonomously → exfiltration, corruption, and destruction risks; (2) email-access chain — email access = every account that resets through it can be compromised; each risk accepted for a chosen interval using the no-deny mechanic; graph sliced into stakeholder, assets, and workflows views; runs without an LLM (curated scenarios); next phase adds external stakeholders and the right-to-delegate question; first validation case for Risk Mandate.ai; effort: moderate | 06/24 personal-scenario-brief |
