# Tools — Proposed Items Index

**Domain:** tools/proposed/ | **Last updated:** 2026-07-20 | **Maintained by:** Librarian (daily run)

All items below are PROPOSED. None have been code-verified. Do not describe any of these as existing features.

This file is a table of contents. Full proposed items live in the topic files below.

---

## Topic Files

| File | Coverage | P-numbers |
|------|----------|-----------|
| [video-editing.md](video-editing.md) | sg-tree, Video Editor Expansion, video crop/overlay/capture, Video Playback, Video Generation | none assigned |
| [wasm-tools.md](wasm-tools.md) | sg-wasm lifecycle, sg-audio-transcription (Whisper), Pyodide sandbox, sg-public-viewer | none assigned |
| [llm-components.md](llm-components.md) | sg-llm component family, Agentic LLM suite, One-Shot IDE, Composite Tools, Infographic v0.1.1 + v2, News Report Tool | none assigned |
| [platform-tools.md](platform-tools.md) | Infrastructure/IFD, Social Previews, Audio tool distribution, TUI API, SG/Edge TUI, SG Labs Admin, SG Mail, Backup and Restore | P-175–P-189, P-196, P-200–P-203, P-229–P-237 |

---

## Quick Reference — P-Number Inventory (Tools domain)

| P-range | Feature group | File |
|---------|---------------|------|
| P-175–P-182 | SG Mail (email client on vaults) | platform-tools.md |
| P-183–P-189 | Backup and Restore Infrastructure | platform-tools.md |
| P-196 | SG Labs Admin Interface | platform-tools.md |
| P-200–P-201 | SG/Edge TUI (Textual + Rich) | platform-tools.md |
| P-202–P-203 | TUI API (structured surface) | platform-tools.md |
| P-229–P-237 | Audio Tool Distribution (release shell + key acquisition) | platform-tools.md |

*All items without P-numbers are from pre-P-numbering briefs. P-numbers were assigned
starting from doc 435+ (05/17 session). Items from earlier sections (17–23) were catalogued
before P-numbering was introduced.*

---

## Full Content Reference

Full content for all items: source monolith at `../v0.16.26__what-exists-today.md`
(Sections 17–32; relevant sub-sections noted in each topic file).
