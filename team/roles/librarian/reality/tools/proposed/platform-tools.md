...
| P-VNT-006 | OpenRouter guardrail enforcement for privacy tiers | Three guardrails on the OpenRouter API key that upgrade privacy tiers from advisory to enforced: (1) model restriction — allowlist of specific model identifiers; (2) provider restriction — allowlist of providers with specified data policies; (3) data policy restriction — enforce no-training, no-logging terms; all three must be active simultaneously for a tier to be enforced rather than advisory | 07/28 voice-note-tool-build-status-brief |
| P-VNT-007 | Product vault (published, seeded key — four safeguards) | The product vault is published by design; its key is published (seeded) and safe only while all four safeguards are active simultaneously: cap (usage ceiling), lifetime (expiry date), revocation (ability to re-issue), narrow reach (read-only scope); if any one safeguard is removed the published key becomes a liability | 07/28 voice-note-tool-build-status-brief |
| P-VNT-008 | Admin vault (secret — key MUST NOT be in any public repo) | Second vault for secrets and key management; entirely separate from the product vault; its vault key must never appear in any committed file; pre-commit hook enforcement (P-VNT-010) is a blocking prerequisite before this vault is created | 07/28 voice-note-tool-build-status-brief |
| P-VNT-009 | Secret distribution via send capability | Distribution of secrets (e.g. replacement admin vault keys) reuses the existing SGraph Send encrypted-transfer capability; avoids out-of-band channels for key rotation | 07/28 voice-note-tool-build-status-brief |
| P-VNT-010 | Pre-commit vault key enforcement hook | A pre-commit Git hook that scans staged files for patterns matching vault key format; rejects the commit with a clear error message if a key is found; must be implemented and active before the admin vault (P-VNT-008) is created; blocking prerequisite | 07/28 voice-note-tool-build-status-brief |
