...
| **`sg.vault.mounts()` (app-facing) returns the RAW `KernelParent.list()` row with no permission check** — incl. `sync.head/named/serverHead` and `sync.lastError.message` (server error text); `sync` is refreshed only by the tab-focus behind-check (30 s debounce), never by a write | **EXISTS** (to be narrowed to a projection) | `app-shell.js` `vAct === 'mounts'`; `kernel-parent.js` `list()`, `status()`, `sync()`; AppSec decision 09/08 |
...
| `sg.fs.mkdir` / `move` / `delete` on a mounted path — the `fs` bridge block calls the data source directly (composite → `EMOUNT_RO`); only `vfs.*` is mount-aware | **EXISTS** (gap) | `app-shell.js` fs mutation block; AppSec decision 09/08 |
| Write receipt `{path, size, commit_id, published}` from `sg.vfs.write`; `fs.mkdir` + same-mount `fs.move` through the mount; child `vfs.move` handler; `SecureChannel.request({timeoutMs})`; app-facing mount-row projection `{prefix, access, declared, state, at}`; shadow pre-flight event; `Via-Mount … parent=<id>`; `sg.append.configure` enum-key derivation by default with a public-read guard | **PROPOSED** | AppSec decision 09/08 (safe list S1–S10) |
