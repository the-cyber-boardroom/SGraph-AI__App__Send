...
**Implementation status (branch `claude/exciting-brown-G2P9Z`, Phase 0 started — still PROPOSED overall):** built so far — `_common/js/lib/links/vault-links.js` (the `*.link.json` convention reader; **unit test 26/26**), and a generic **lazy-on-expand** enhancement to the shared `send-browse--v0.3.2.js` (Approach B: `_lazy`/`_folderPath` nodes call `dataSource.loadFolder()`+re-render on first expand; `_subvault` glyph+chip; backward-compatible, share/open trees unaffected). **Not yet built:** `CompositeDataSource` + `vault-shell` wiring (the end-to-end sub-vault open/splice), the link card, owner records, external embeds. **Not browser-verified** (no browser in the build env). Plan: `team/roles/dev/reviews/05/25/v0.27.62__implementation-plan__sub-vaults-and-external-resources.md`.
