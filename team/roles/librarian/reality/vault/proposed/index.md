...
| P-177 | RO-token entry on the `/en-gb/app/<public-id>` key prompt | The preview card's key prompt (`sg-public-preview-card.js`) now invites a read-only token alongside the vault key (updated placeholder + hint `ro-…`). The opener path `app-shell._initWithKey` already branches on `key.startsWith('ro-')` → `_resolveROToken` → `SGVault.openReadOnly` (read-only, wrong-vault guard applies; not persisted as `sg-vault-key`). **BLOCKER (code-verified):** the RO round-trip does not resolve today — both `app-shell._resolveROToken` and `vault-loader.openROToken` expect `GET /api/transfers/check-token/<token>` to return `{ ciphertext }`, but the backend route returns only `{valid,status,remaining}` and the payload lives at `/download/<transfer_id>`; `vault-token-manager.createTransferToken` also lets the server assign a *random* transfer-id, so a recipient holding only `ro-word-word-NNNN` has no way to find it. Fix (proposed): a deterministic RO transfer-id derived from the token (the proven public-preview pattern) used by BOTH create and resolve, dropping the `check-token`-ciphertext dependency (client-only, no backend change) | lead clarification (enter ro token in pic1) |
