...
**No agent ever sees a secret that isn't encrypted for their key.** Even if all agents run on the same machine, in the same process, with shared memory — the cryptographic boundary enforces isolation. A prompt injection in the Research Agent cannot access INTERNAL_PRICING_DATA because it's encrypted for the Analysis Agent's key, and the Research Agent doesn't have that key.
...
|---|---|
| **Encrypted secret isolation** | Compromised agent cannot access other agents' secrets |
...
Step 3: What the attacker CANNOT access:
  → Internal pricing data (encrypted for Analysis Agent — different key)
  → Compliance rulebook (encrypted for Compliance Agent — different key)  
  → Other agents' outputs (encrypted for their respective keys)
  → The orchestrator's master context
