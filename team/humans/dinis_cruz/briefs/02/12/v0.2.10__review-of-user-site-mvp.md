...
- create a briefing with all these issues and try to visualise them in tables and graphs (using AsciiArt and mermaid diagrams)
...
- if you don't have any questions, then start applying those fixes (one phase/group at the time). As you fix (or have an update) an issue, please update the .issues db, and the source code reference (i.e. the comment you updated above)
...
- great work in writing all those tests :)
...
- UPDATE: when I deployed this to Lambda (exposed via FunctionURL), it worked !!!
  - the file upload worked
  - I was able to copy the link given and paste it in another window
  - I then provided the decryption key
  - and the file was downloaded and decrypted ok :)
- one issue that this has (by design) is that this only works if we hit the same lambda function (which is the one that stored the file in memory), so in most cases we can upload but can't download (but I was able to get it working once, and have the screenshots to prove it, which proves the full end to end flow :) :) :) ) . I posted about this first MVP on LinkedIn here: https://www.linkedin.com/feed/update/urn:li:ugcPost:7427535451630276608/ (has images of the MVP working end-to-end)
- this will be addressed once we add the Memory-FS and its S3 Storage (which I have the code for already)
- we need to figure out why it doesn't work locally (i.e locally I get the "Cannot read properties of undefined (reading 'generateKey')"), but on the same browser (different tab) the lambda function URL one worked
