...
- one issue that this has (by design) is that this only works if we hit the same lambda function (which is the one that stored the file in memory), so in most cases we can upload but can't download (but I was able to get it working one which proves the full end to end flow :) :) :) )
