...
3. **Friendly tokens** — instead of a 64-character hex URL, users share `coral-equal-1774` — a human-readable token derived via SHA-256 + PBKDF2 that encodes both the transfer ID and decryption key. 320 × 320 × 10,000 = ~1 billion combinations.
