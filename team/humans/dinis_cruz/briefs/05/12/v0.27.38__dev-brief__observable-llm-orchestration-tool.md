...
The conversation graph is stored as a vault. Each conversation is a vault. Each turn is a commit. The graph state is versioned. The compression decisions are auditable.

This means:

- A conversation can be shared (vault read key)
...
The visibility and graph features change what is possible:
...
For me personally, this replaces:

- Claude desktop for workflows I want full visibility on (still use Claude desktop for one-off questions)
- The cobbled-together logging I have around local model runs
- The "screenshot the conversation to remember it" workflow
- The "rewrite the prompt by hand because I do not trust the compression" workflow

For the platform broadly, this becomes one of the canonical SG/Apps. A demonstration of what the SG/App framework enables and a useful tool in its own right.

## Build Sequence

The tool is large. Build in this order:
...
|---|---|---|
| 11 May | `v0.27.32__dev-brief__local-claude-milestone.md` | The local model setup this tool will orchestrate |
| 02 May | `v0.27.5__arch-brief__sg-app-sandboxed-javascript-applications.md` | The tool is built as an SG/App |
| 02 May | `v0.27.5__dev-brief__ontologist-semantic-knowledge-graphs.md` | Same graph-extraction pattern, different domain (content vs conversations) |
| 22 Apr | `v0.22.18__arch-brief__observability-pipeline-capture-transform-project.md` | LETS pipeline; this tool is LETS for LLM conversations |
| 21 Apr | `v0.21.9__dev-brief__task-focused-llm-components.md` | The seven LLM components; this tool unifies them |
...
| 10 | Conversations are stored as vaults | Each conversation is a vault; commit history shows every turn |
