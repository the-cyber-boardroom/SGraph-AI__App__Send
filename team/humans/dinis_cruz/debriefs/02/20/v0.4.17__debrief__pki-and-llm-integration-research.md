...
Build a `sigsend` CLI tool and MCP server so any LLM can encrypt and upload files. Includes trust model analysis, package structure, and code sketches.
...
2. **Package name:** `sigsend` or `sg-send` on PyPI?
...
- **If CLI/MCP first:** Build `sigsend` crypto module, validate format compatibility with `crypto.js`, implement upload/download commands, package for PyPI, then wrap in MCP server
