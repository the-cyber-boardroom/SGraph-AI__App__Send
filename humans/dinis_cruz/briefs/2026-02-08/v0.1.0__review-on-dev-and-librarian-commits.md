...
- we are going to use Memory-FS for simulating (and have support) for a generic data store. this means that we could have a data store that works: in memory, in a zip file, in a mysql file, and in S3. We are going to use Memory-Fs in single file mode (see docs in library/dependencies/memory-fs) 
...
- We will be using OSBot-Fast-API-Serverless package (which I will wire up to the project) and already supports a header/cookie based auth (each server can define an key-name and key-value that are check on all requests (apart from the /docs one))
...
- The Type_Safe classes/architecture/approach sits at the heart of every code pattern (since it provides run-time type safety and many more powerful development benefits). See the guidance in ./library/dependencies/osbot-utils/type-safe
...
- we are not going to do use a Base path like "api/v1/" at a later/any stage. What we will be doing is to will be to create a python client API that will provide the abstraction layer between the REST API and the actually methods a consumer of that API will want to use (please add this roadmap since we don't need this just yet), which means that we don't need to have 'protective' paths patterns like /v1 (see ./library/guides/patterns/v1.1.4___v0.16.1__cache_service__testing_guidance.md for how I use this pattern in another project)
...
- Timestamps: all timestamps to use are ints, and provided by the class : from osbot_utils.type_safe.primitives.domains.identifiers.safe_int.Timestamp_Now            import Timestamp_Now
- transfer_id, token_id and admin_key are all Obj_Id (from osbot_utils.type_safe.primitives.domains.identifiers.Obj_Id import Obj_Id) and should be setup just like Node_Id (from OSBot_Utils). Important node , we need to make sure that Transfer_Id support's empty values, which are not supported by Obj_id (see how Node_Id does it). add an Transfer_Id.new() helper method for the cases when we need a new random value
...
- on the IFD 'admin panel' mentioned above, see/read the files in ./library/guides/development/ifd/ifd__issues-fs which were created from how this was done in the Issues-Fs project (start reading on v0.1.8__ifd__README.md) 

## on roles/architect/v0.1.1/v0.1.1__issues-fs-improvements.md

1 ) "Missing: "Phase" as a First-Class Node Type" -  add it to the node-types.json file (which was missing, so I just added a default on to this project). The whole idea of the Isses-Fs fractal nature of the issues types is that each project or even issue, can define its own issue-types and issue-links
...
## on roles/dev/briefs/v0.1.1__sgraph-send-plan-mode-to-review.md 
(this is the file created by Dev)

- ok I just review it again (I had read it before I wrote the comments above), and this needs to be recreated (and split into separate documents) based on the comments and decisions I made above



## for next version
- both Claude and Code, please review this data in context of your current role, create files with your reviews in folders inside the respective folders (for example related to you , use the role/* folder)
- all files you create should be prefixed with the curent version (which should be v0.1.2 or higher)
- for your review, create a master file that provides an overview of everything you created and provide links (and tables) to the files created (this will be the first file I should read)
- please create a separate file for questions to me, or questions to each other (one file per target role/agent)