...
# Publish the app version as a plain-text /version file at the serve root, mirroring what
# deploy_static_site.py uploads in production. vault-header.js fetches /version to show the
# actual running build (CI-incremented), so this lets local dev surface the real version too
# — a reliable way to confirm you're looking at the latest files.
APP_VERSION="local-dev"
if [ -f "$REPO_ROOT/sgraph_ai_app_send/version" ]; then
    APP_VERSION="$(tr -d '[:space:]' < "$REPO_ROOT/sgraph_ai_app_send/version")"
    cp "$REPO_ROOT/sgraph_ai_app_send/version" "$SERVE_DIR/version"
