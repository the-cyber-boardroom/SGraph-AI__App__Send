...
# Read the app version from sgraph_ai_app_send/version so the local build-info.js stamps
# the real CI-incremented value (instead of 'local-dev'). vault-header.js renders this in
# the overflow menu, giving a reliable way to confirm you're looking at the latest files.
APP_VERSION="local-dev"
if [ -f "$REPO_ROOT/sgraph_ai_app_send/version" ]; then
    APP_VERSION="$(tr -d '[:space:]' < "$REPO_ROOT/sgraph_ai_app_send/version")"
fi

# Inject build-info.js (CI generates this in production via inject_build_version.py).
mkdir -p "$SERVE_DIR/_common/js"
