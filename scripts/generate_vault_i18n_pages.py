#!/usr/bin/env python3
"""Generate locale-specific HTML pages for SGraph Vault UI.
...
Forked from generate_send_i18n_pages.py — same approach, vault-specific config.

Usage:
    python scripts/generate_vault_i18n_pages.py
    python scripts/generate_vault_i18n_pages.py --dry-run
    python scripts/generate_vault_i18n_pages.py --version v0.1.0
"""
import json
import os
import re
...
UI_BASE    = Path(__file__).parent.parent / 'sgraph_ai_app_send__ui__vault'
SOURCE_DIR = 'en-gb'

# Locale definitions — matches main sgraph.ai website convention exactly.
LOCALES = [
...
def translate_html(html_content, translations, en_translations, locale_info):
    ...
    # 2. Add pre-rendered marker so vault-i18n.js knows to skip JSON loading
    ...
    # 4. Translate data-i18n-placeholder attribute values
    ...
    # 5. Translate inline VaultI18n.t() calls in template literals
...
def main():
    ...
    print(f"SGraph Vault — i18n Page Generator")
    ...
    if not i18n_dir.is_dir():
        ...
        print(f"  No locale translations available — only en-gb/ will be deployed.")
        return 0
    ...
    for loc in LOCALES:
        ...
        if t:
            ...
            print(f"  {loc['slug']:6s} SKIPPED (no translation file)")
    print()

    if not available_locales:
        print(f"  No locale translations found — nothing to generate.")
        return 0
    ...
    print(f"  Done. {'Would generate' if args.dry_run else 'Generated'} {generated} files "
          f"across {len(available_locales)} locales.")
    return 0
